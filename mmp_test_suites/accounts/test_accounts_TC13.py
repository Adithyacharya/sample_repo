...
def test_resend_invite_to_activated_user(chrome_driver, logger, screen_shots_list):
    try:
        file = os.path.basename(os.path.abspath(__file__))
        config = read_data_from_config(file)
        login_user_to_mmp(chrome_driver)
        logger.info("********* TC13 : Resend Invite to Active account ********")
        ...
        account_page.enter_value_to_perform_search(
            config["TC13"]["resend_invite_email"]
        )
        ...
        test_case_data = config["TC13"]["post_account_status"]
        custom_assert(
