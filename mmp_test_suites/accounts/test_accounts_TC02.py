...
def test_add_new_account(chrome_driver, logger, screen_shots_list):
    ...
    logger.info("******TC2:Clicked on Add accounts page********")

    # Launch Add Accounts Page
    add_account_page = AddAccountPage(chrome_driver)
    logger.info(
        "********* TC2:Launching Add Accounts Page and adding accounts *************"
    )
    add_account_page.add_account_enter_first_name(config["TC2"]["fname"])
    add_account_page.add_account_enter_last_name(config["TC2"]["lname"])
    add_account_page.add_account_enter_email_id(config["TC2"]["email_id"])
    ...
    add_account_page.add_account_assign_sls_number(config["TC2"]["mml_number2"])
    add_account_page.add_account_assign_forward_number(config["TC2"]["fwd_number"])
    time.sleep(5)
    ...
    with check:
        custom_assert(
            mmp_email_fplace == config["TC2"]["fname"] + " " + config["TC2"]["lname"],
    ...
    with check:
        custom_assert(
            mmp_email_id == config["TC2"]["email_id"],
    ...
    email_in_db = config["TC2"]["email_id"]
    with check:
        custom_assert(
            len(result_set) != 0,
    ...
    ref_fname_value = config["TC2"]["fname"]
    with check:
        custom_assert(
    ...
    ref_lname_value = config["TC2"]["lname"]
    with check:
        custom_assert(
