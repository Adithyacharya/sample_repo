...
from pages.accounts.edit_line_page import EditLinePage
...
def test_delete_account_from_edit_line_page(chrome_driver, logger, screen_shots_list):
    try:
        file = os.path.basename(os.path.abspath(__file__))
        config = read_data_from_config(file)
        login_user_to_mmp(chrome_driver)
        logger.info("********* TC18 : Delete Account from edit line page ********")

        account_page = AccountPage(chrome_driver)
        nav_menu = NavMenuPage(chrome_driver)
        delete_pop_up = DeleteAccountPopUp(chrome_driver)
        edit_line_page = EditLinePage(chrome_driver)

        nav_menu.go_to_accounts_tab()
        account_page.enter_value_to_perform_search(config["TC18"]["email_to_delete"])
        account_page.click_on_search_button()
        account_page.click_on_edit_line()
        edit_line_page.click_on_delete_button_from_edit_line_page()
        delete_pop_up.enter_tag_name(config["TC18"]["tag"])
        delete_pop_up.click_proceed_button()
        logger.info("Deleted User using delete option in Edit line page")
        time.sleep(3)
        ...
        email_param = config["TC18"]["email_to_delete"]
        mml_param = config["TC18"]["mml_number"]
        device_number_param = config["TC18"]["device_number"]
