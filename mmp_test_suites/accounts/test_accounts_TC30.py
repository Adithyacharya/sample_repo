...
def test_social_messaging_filter(chrome_driver, logger, screen_shots_list):
    file = os.path.basename(os.path.abspath(__file__))
    logger.info("********* TC30 : Social Messaging_Filter ********")
    ...
    logger.info("Check for Social Messaging filter on")
    account_filter.click_on_social_messaging_filter()
    account_filter.click_on_social_messaging_filter_on()
    if (account_page.check_first_account_in_accounts() == True):
        ...
        time.sleep(10)
        flag = account_filter.get_checked_attribute_from_social_messaging()
        with check:
            custom_assert(
                flag == 'true',
    ...
    account_filter.click_filter()
    account_filter.click_active_filter()
    logger.info("Check for Social Messaging filter off")
    account_filter.click_on_social_messaging_filter()
    account_filter.click_on_social_messaging_filter_off()
    if (account_page.check_first_account_in_accounts() == True):
        account_page.click_on_first_account_in_accounts()
        account_page.click_on_edit_account_line()
        time.sleep(10)
