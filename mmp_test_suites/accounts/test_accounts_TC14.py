...
def test_resend_invite_to_not_invited_user(chrome_driver, logger, screen_shots_list):
    try:
        file = os.path.basename(os.path.abspath(__file__))
        config = read_data_from_config(file)
        login_user_to_mmp(chrome_driver)
        logger.info("********* TC14 : Resend Invite to not invited account ********")
        ...
        account_page.enter_value_to_perform_search(
            config["TC14"]["resend_invite_email"]
        )
        ...
        test_case_data = config["TC14"]["post_account_status"]
        custom_assert(
