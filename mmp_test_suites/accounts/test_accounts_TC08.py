...
def test_add_new_account(chrome_driver, logger, screen_shots_list):
    ...
    logger.info("****** TC8:Clicked on Add accounts page ********")

    add_account_page.click_multiple_accounts_tab()
    logger.info("****** TC8:Clicked on Multiple accounts tab ********")
    ...
    file_path = os.path.join(
        os.getcwd(), "templates", config["meta_info"]["file_path4"]
    )
    multi_accounts.upload_file_button(file_path)
    logger.info("*******TC8: Uploaded accounts CSV file ********")
    time.sleep(1)
    ...
    with check:
        custom_assert(
            mmp_email_fplace == config["TC8"]["fname"] + " " + config["TC8"]["lname"],
    ...
    with check:
        custom_assert(
            mmp_email_id == config["TC8"]["email_id"],
    ...
    email_in_db = config["TC8"]["email_id"]
    with check:
        custom_assert(
            len(result_set) != 0,
    ...
    ref_fname_value = config["TC8"]["fname"]
    with check:
        custom_assert(
    ...
    ref_lname_value = config["TC8"]["lname"]
    with check:
        custom_assert(
