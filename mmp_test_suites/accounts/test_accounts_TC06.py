...
def test_add_new_account(chrome_driver, logger, screen_shots_list):
    ...
    logger.info("****** TC6:Clicked on Add accounts page ********")

    # Launch Add Accounts Page
    add_account_page.click_multiple_accounts_tab()
    logger.info("****** TC6:Clicked on Multiple accounts tab ********")
    ...
    file_path = os.path.join(
        os.getcwd(), "templates", config["meta_info"]["file_path2"]
    )
    multi_accounts.upload_file_button(file_path)
    time.sleep(1)
    logger.info("******* TC6:Uploaded the accounts CSV file ********")
    multi_accounts.click_import_button()
    account_page.click_accounts_options_from_nav_menu(logger)
    time.sleep(3)
    mmp_email_fplace = add_account_page.check_account_presence_first_place()
    with check:
        custom_assert(
            mmp_email_fplace == config["TC6"]["fname"] + " " + config["TC6"]["lname"],
    ...
    with check:
        custom_assert(
            mmp_email_id == config["TC6"]["email_id"],
    ...
    email_in_db = config["TC6"]["email_id"]
    with check:
        custom_assert(
            len(result_set) != 0,
    ...
    ref_fname_value = config["TC6"]["fname"]
    with check:
        custom_assert(
    ...
    ref_lname_value = config["TC6"]["lname"]
    with check:
        custom_assert(
