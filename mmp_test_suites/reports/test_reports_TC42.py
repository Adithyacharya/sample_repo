...
def test_voice_activity_from_to_ml_numbers(chrome_driver, logger, screen_shots_list):
    try:
        file = os.path.basename(os.path.abspath(__file__))
        config = read_data_from_config(file)
        login_user_to_mmp(chrome_driver)
        logger.info(
            "********* TC42 : Voice Activity filter using From & To ML Numbers ********"
        )
        ...
        from_ml = config["TC42"]["from_ml"]
        to_ml = config["TC42"]["to_ml"]
        voice_activity_page.enter_from_to_ml_numbers(from_ml, to_ml)
        ...
        report_file_name = config["TC42"]["report_file_name"]
        ...
        headers = config["TC42"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC42"]["report_type"]
        ...
        # Validate All the data has calling number & called number as the from & to ML numbers
        for voice_data in data_dict_list:
            assert (from_ml.lower() in voice_data["Calling Number"].lower()) and (
                to_ml.lower() in voice_data["Called Number"].lower()
            ), f"Validation Failed, as downloaded csv doesn't have the keyword searched"
