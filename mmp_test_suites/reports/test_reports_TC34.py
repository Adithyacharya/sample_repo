...
def test_voice_activity_calls_to_vm_call_type_tdm_call_mode(
    ...
    try:
        ...
        logger.info(
            "********* TC34 : Voice Activity filter using Call Type : Calls to VM & Call Mode : TDM ********"
        )
        ...
        voice_activity_page.select_call_type_drop_down("ctv")
        ...
        report_file_name = config["TC34"]["report_file_name"]
        ...
        headers = config["TC34"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC34"]["report_type"]
