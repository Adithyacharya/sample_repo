...
def test_consumption_metrics_data_message_for_today(chrome_driver, logger, screen_shots_list):
    logger.info(
        "********* Reports-TC07 : Consumption Metrics_Message for Today ********")
    file = os.path.basename(os.path.abspath(__file__))
    config = read_data_from_config(file)
    report_file_name = config["TC07"]["report_name"]
