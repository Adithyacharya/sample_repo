...
def test_voice_activity_calls_made_call_type_tdm_call_mode(
    ...
    try:
        ...
        logger.info(
            "********* TC31 : Voice Activity filter using Call Type : Calls Made & Call Mode : TDM ********"
        )
        ...
        voice_activity_page.select_call_type_drop_down("cm")
        ...
        report_file_name = config["TC31"]["report_file_name"]
        ...
        headers = config["TC31"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC31"]["report_type"]
