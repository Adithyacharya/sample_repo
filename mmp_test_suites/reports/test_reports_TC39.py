...
def test_voice_activity_keyword_name(chrome_driver, logger, screen_shots_list):
    try:
        file = os.path.basename(os.path.abspath(__file__))
        config = read_data_from_config(file)
        login_user_to_mmp(chrome_driver)
        logger.info(
            "********* TC39 : Voice Activity filter using Keyword as Name ********"
        )
        ...
        keyword_text = config["TC39"]["keyword_name"]
        voice_activity_page.enter_keyword(keyword_text)
        ...
        report_file_name = config["TC39"]["report_file_name"]
        ...
        headers = config["TC39"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC39"]["report_type"]
        ...
        # Validate All the data has calling number & called number as the testdata
        for voice_data in data_dict_list:
            assert (keyword_text.lower() in voice_data["Calling Number"].lower()) or (
                keyword_text.lower() in voice_data["Called Number"].lower()
