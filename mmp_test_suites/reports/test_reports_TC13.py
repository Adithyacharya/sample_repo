...
import time
import os
from utils.commons import (read_data_from_config, getFormattedDate, rename_downloaded_report,
                           read_content_from_csv, custom_assert, getTodayDate)


def test_consumption_metrics_data_for_7_days(chrome_driver, logger, screen_shots_list):
    logger.info(
        "********* Reports-TC13 : Consumption Metrics_Voice data for Last 7 days ********")
    file = os.path.basename(os.path.abspath(__file__))
    config = read_data_from_config(file)
    report_file_name = config["TC13"]["report_name"]
