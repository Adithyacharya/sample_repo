...
def test_voice_activity_calls_to_vm_call_type_all_call_mode(
    ...
    try:
        ...
        logger.info(
            "********* TC33 : Voice Activity filter using Call Type : Calls to VM & Call Mode : ALL ********"
        )
        ...
        report_file_name = config["TC33"]["report_file_name"]
        ...
        headers = config["TC33"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC33"]["report_type"]
