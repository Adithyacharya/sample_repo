...
def test_voice_activity_calls_made_call_type_all_call_mode(
    ...
    try:
        ...
        logger.info(
            "********* TC30 : Voice Activity filter using Call Type : Calls Made & Call Mode : ALL ********"
        )
        ...
        report_file_name = config["TC30"]["report_file_name"]
        ...
        headers = config["TC30"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC30"]["report_type"]
