...
from utils.commons import (
    getFormattedDate, read_data_from_config, rename_downloaded_report, read_content_from_csv, custom_assert, getTodayDate)


def test_consumption_metrics_data_for_today(chrome_driver, logger, screen_shots_list):
    logger.info(
        "********* Reports-TC12 : Consumption Metrics_Voice for Today ********")
    file = os.path.basename(os.path.abspath(__file__))
    config = read_data_from_config(file)
    report_file_name = config["TC12"]["report_name"]
