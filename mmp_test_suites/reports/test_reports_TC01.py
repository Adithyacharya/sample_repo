...
def test_consumption_metrics_data_voice_for_custom_date(chrome_driver, logger, screen_shots_list):
    logger.info(
        "********* Reports-TC01 : Consumption Metrics_Voice for Custom Date ********")
    file = os.path.basename(os.path.abspath(__file__))
    config = read_data_from_config(file)
    report_file_name = config["TC01"]["report_name"]
