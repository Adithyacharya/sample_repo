...
def test_message_activity_whatsapp_messages_sent_type(
    ...
    try:
        ...
        logger.info(
            "********* TC51 : Message Activity filter using Type as Whatsapp Messages Sent ********"
        )
        ...
        message_activity_page.update_wait_time(20)
        message_activity_page.select_message_type("WhatsApp Messages Sent")
        ...
        report_file_name = config["TC51"]["report_file_name"]
        ...
        headers = config["TC51"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]

        logger.info("Header list in CSV Expected " + str(test_case_data))
        logger.info(
            "Header list fetched from CSV downloaded " +
            str(fetched_header_list_csv)
        )
        with check:
            assert (
        ...
        logger.info("Report Name in CSV downloaded " +
                    str(report_name_from_csv))
        with check:
            assert (
        ...
        test_case_data = config["TC51"]["report_type"]
        ...
        for data in data_dict_list:
            ...
            assert (
                "whatsapp" in message_platform.lower()
            ), f"Validation Failed as the Message Platform Obtained {message_platform} but expected Whatsapp"
