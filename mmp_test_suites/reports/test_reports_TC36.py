...
def test_voice_activity_dropped_call_call_type_all_call_mode(
    ...
    try:
        ...
        logger.info(
            "********* TC36 : Voice Activity filter using Call Type : Dropped call & Call Mode : ALL ********"
        )
        ...
        report_file_name = config["TC36"]["report_file_name"]
        ...
        headers = config["TC36"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC36"]["report_type"]
