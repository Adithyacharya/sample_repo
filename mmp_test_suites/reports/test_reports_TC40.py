...
def test_voice_activity_keyword_email(chrome_driver, logger, screen_shots_list):
    try:
        file = os.path.basename(os.path.abspath(__file__))
        config = read_data_from_config(file)
        login_user_to_mmp(chrome_driver)
        logger.info(
            "********* TC40 : Voice Activity filter using Keyword as Email ********"
        )
        ...
        keyword_text = config["TC40"]["keyword_email"]
        ...
        report_file_name = config["TC40"]["report_file_name"]
        ...
        headers = config["TC40"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC40"]["report_type"]
