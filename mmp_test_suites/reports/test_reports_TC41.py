...
def test_voice_activity_keyword_device_number(chrome_driver, logger, screen_shots_list):
    try:
        file = os.path.basename(os.path.abspath(__file__))
        config = read_data_from_config(file)
        login_user_to_mmp(chrome_driver)
        logger.info(
            "********* TC41 : Voice Activity filter using Keyword as Device Number ********"
        )
        ...
        keyword_text = config["TC41"]["keyword_device_number"]
        ...
        report_file_name = config["TC41"]["report_file_name"]
        ...
        headers = config["TC41"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC41"]["report_type"]
