...
def test_message_activity_all_sms_message_type(
    ...
    try:
        ...
        logger.info(
            "********* TC46 : Message Activity filter using Type as All SMS Messages ********"
        )
        ...
        message_activity_page.select_message_type("All SMS Messages")
        ...
        report_file_name = config["TC46"]["report_file_name"]
        ...
        headers = config["TC46"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC46"]["report_type"]
