...
def test_voice_activity_calls_made_call_type_tdm_call_mode(
    ...
    try:
        ...
        logger.info(
            "********* TC32 : Voice Activity filter using Call Type : Calls Made & Call Mode : Data ********"
        )
        ...
        report_file_name = config["TC32"]["report_file_name"]
        ...
        headers = config["TC32"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC32"]["report_type"]
