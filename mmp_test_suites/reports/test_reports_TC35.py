...
def test_voice_activity_calls_to_vm_call_type_tdm_call_mode(
    ...
    try:
        ...
        logger.info(
            "********* TC35 : Voice Activity filter using Call Type : Calls to VM & Call Mode : Data ********"
        )
        ...
        report_file_name = config["TC35"]["report_file_name"]
        ...
        headers = config["TC35"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC35"]["report_type"]
