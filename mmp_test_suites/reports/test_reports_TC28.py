...
def test_voice_activity_calls_received_call_type_tdm_call_mode(
    ...
    try:
        ...
        logger.info(
            "********* TC28 : Voice Activity filter using Call Type : Calls Received & Call Mode : TDM ********"
        )
        ...
        report_file_name = config["TC28"]["report_file_name"]
        ...
        headers = config["TC28"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC28"]["report_type"]
