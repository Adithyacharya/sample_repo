...
def test_voice_activity_dropped_call_call_type_tdm_call_mode(
    ...
    try:
        ...
        logger.info(
            "********* TC38 : Voice Activity filter using Call Type : Dropped call & Call Mode : Data ********"
        )
        ...
        report_file_name = config["TC38"]["report_file_name"]
        ...
        headers = config["TC38"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC38"]["report_type"]
