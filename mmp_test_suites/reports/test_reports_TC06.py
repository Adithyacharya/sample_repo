from utils.commons import login_user_to_mmp, read_data_from_config
...
def test_consumption_metrics_data_message_for_custom_date(chrome_driver, logger, screen_shots_list):
    logger.info(
        "********* Reports-TC06 : Consumption Metrics_Message for Custom Date ********")
    file = os.path.basename(os.path.abspath(__file__))
    config = read_data_from_config(file)
    report_file_name = config["TC06"]["report_name"]
