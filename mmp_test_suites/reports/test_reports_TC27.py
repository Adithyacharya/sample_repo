...
def test_voice_activity_calls_received_call_type_all_call_mode(
    ...
    try:
        ...
        logger.info(
            "********* TC27 : Voice Activity filter using Call Type : Calls Received & Call Mode : ALL ********"
        )
        ...
        report_file_name = config["TC27"]["report_file_name"]
        ...
        headers = config["TC27"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC27"]["report_type"]
