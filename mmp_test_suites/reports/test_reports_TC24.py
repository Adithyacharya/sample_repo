...
def test_voice_activity_all_call_type_all_call_mode(
    ...
    try:
        ...
        logger.info(
            "********* TC24 : Voice Activity filter using Call Type : All & Call Mode : All********"
        )
        ...
        report_file_name = config["TC24"]["report_file_name"]
        ...
        headers = config["TC24"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC24"]["report_type"]
