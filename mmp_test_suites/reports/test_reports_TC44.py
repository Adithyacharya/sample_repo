...
def test_voice_activity_all_call_type_all_call_mode(
    ...
    try:
        ...
        logger.info(
            "********* TC44 : Voice Activity filter using Call Duration ********"
        )
        ...
        voice_activity_page.enter_min_and_max_call_duration(0, 1)
        ...
        report_file_name = config["TC44"]["report_file_name"]
        ...
        headers = config["TC44"]["header_list"]
        test_case_data = [str(header.strip()) for header in headers.split(",")]
        ...
        test_case_data = config["TC44"]["report_type"]
        ...
        # Validate the obtained csv has only records with in the call duration

        for voice_data in data_dict_list:
            fetched_duration = voice_data["Duration"].split(":")[0]
            assert (
                0 <= int(fetched_duration) <= 1
            ), "Validation failed as there are records which doesn't comply with call duration filter"
